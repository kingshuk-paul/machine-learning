...
J_mat = y_new .* log(a3) + (1 - y_new) .* log (1 - a3);
J = -1/m * sum(sum(J_mat, 1));

Theta1_noBias = Theta1( : , 2:size(Theta1,2));
Theta2_noBias = Theta2( : , 2:size(Theta2,2));

regCost = lambda/ (2 * m) * (sum(sum( Theta1_noBias .* Theta1_noBias, 1) ) + sum(sum( Theta2_noBias .* Theta2_noBias, 1))) ;
J = J + regCost;
